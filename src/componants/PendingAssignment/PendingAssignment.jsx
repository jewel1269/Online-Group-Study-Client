...
const PendingAssignment = () => {
  ...
  useEffect(() => {
    axios
      .get("https://online-group-study-server-red.vercel.app/submittedAssignment", {
        ...
        <section className="my-8 dark:bg-gray-100 dark:text-gray-800">
          <div className="container flex flex-col items-center p-4 mx-auto space-y-6 md:p-8">
            ...
            <p className="px-6 py-2 text-2xl font-semibold text-center sm:font-bold sm:text-3xl md:text-4xl lg:max-w-2xl xl:max-w-4xl dark:text-gray-700">
              "Here is The all Pending Assignment"
            </p>
          </div>
        </section>
      </div>

      <div className="lg:grid md:grid md:grid-cols-2 lg:grid-cols-5 gap-5 lg:ml-16 lg:mr-16 mt-10">
        {items.map((item) => (
          <PendingCard key={item._id} item={item}></PendingCard>
        ))}
      </div>
    </div>
  );
